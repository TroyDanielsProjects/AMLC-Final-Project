...
class PodScraper():
    """
    Scrape and store Podcast Data into a folder of text files.
    """
    ...
    def run(self, url, after_date=None):
        ...
        if not os.path.isdir(f"/mnt/gcs/{podcast_name}"):
            os.makedirs(f"/mnt/gcs/{podcast_name}")
    ...
    def write_transcriptions(self, podcast_name):
        for episode in self.ref_dict.keys():
            ...
            file_path = os.path.join((f"/mnt/gcs/{podcast_name}"), f"{episode}|{date.date()}.txt")
